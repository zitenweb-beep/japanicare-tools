...
【漢字↔ひらがな統一ルール】
▼ひらがなで書く語（開く）
・つらい（×辛い）・いただく（×頂く）・さい（×際：「〜のさいに」）
・できる（×出来る）・いたす（×致す）・くださる（×下さる）
・すでに（×既に）・ほとんど（×殆ど）・まだ（×未だ）
・さらに（×更に）・ただし（×但し）・さまざま（×様々）
・ところ（形式名詞：「〜したところ」）・こと（「〜すること」）
・もの（「〜するもの」）・とき（「〜のとき」）・ため（「〜のため」）
・〜してみる（×〜して見る）・〜しておく（×〜して置く）・〜してほしい（×〜して欲しい）
▼漢字で書く語（閉じる）
・引き続き（×ひきつづき）・概ね（×おおむね）・行う（×おこなう）
・見られる（×みられる：「笑顔が見られた」）
・ほかの利用者（×他利用者・他の利用者：「ほかの」はひらがな、ただし「他」は使わない）
・「支援」「作業」「体調」「確認」「報告」「記録」「対応」「促す」「声掛け」「見守り」「把握」"""
            msg = client.messages.create(
                model='claude-sonnet-4-6',
                max_tokens=2000,
                messages=[{'role': 'user', 'content': prompt}]
